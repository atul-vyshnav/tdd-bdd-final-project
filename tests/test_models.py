...
class TestProductModel(unittest.TestCase):
    ...
    def test_deserialize_a_product(self):
        """It should Deserialize a dictionary back into a Product"""
    ...
    def test_deserialize_with_missing_data(self):
        """It should raise DataValidationError when data is missing keys"""
        data = {"name": "Box", "description": "Cardboard"}  # Missing price, available, category
        product = Product()
    ...
    def test_deserialize_with_bad_available_type(self):
        ...
        product = ProductFactory()
        data = product.serialize()
        data["available"] = "true"  # Should be a boolean True/False, not a string
        product = Product()
        with self.assertRaises(DataValidationError):
            product.deserialize(data)
    ...
    def test_find_by_price_string(self):
        """It should Find Products by Price passed as a string string"""
        product = ProductFactory()
        product.price = Decimal("19.99")
        product.create()
        
        # Test passing the price as a string, including spaces/quotes
    ...
    def test_update_without_id_raises_error(self):
        """It should raise DataValidationError if the product has no ID"""
        product = ProductFactory()
        product.id = None  # Force the ID to be empty
        
        # This tells the test runner: "Expect an error on the next line"
        with self.assertRaises(DataValidationError):
            product.update()
